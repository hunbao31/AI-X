...
function TeacherManageInner() {
  ...
  const filteredExercises = exercises.filter((ex) => {
    ...
    if (!q) return true;
    return ex.topic.toLowerCase().includes(q) || ex.question.toLowerCase().includes(q);
  });
    ...
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      {!loading && exercises.length > 0 && (
        <input
          ...
          placeholder="Tìm theo chương/chủ đề hoặc nội dung câu hỏi, VD: Mệnh đề và tập hợp"
      ...
      {loading ? (
        ...
        <Card>
          <p className="text-slate-300">Không tìm thấy câu hỏi nào khớp với "{search}".</p>
        </Card>
      ) : (
